...
class MyViTBlock(nn.Module):
    def __init__(self, weights, bias, hidden_d, n_heads, cpt, mlp_ratio=4, rank=-1):
        ...
        self.mlp = nn.Sequential(
            LowRankLinear(hidden_d, mlp_ratio * hidden_d, weights[2*cpt], bias[2*cpt], rank = rank),
            nn.GELU(),
            LowRankLinear(mlp_ratio * hidden_d, hidden_d, weights[1+2*cpt], bias[1+2*cpt], rank = rank)
        )
