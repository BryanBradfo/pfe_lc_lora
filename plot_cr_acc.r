...
# Ajouter une colonne pour différencier les types de segments
data$line_type <- ifelse(grepl("n_heads = 8", data$model), "dashed", "solid")

# Créer le plot
ggplot(data, aes(x = accuracy, y = compression_ratio, color = model, group = model, linetype = line_type)) +
  ...
  scale_linetype_manual(name = "n_heads", values = c("solid" = "solid", "dashed" = "dashed"),
                        labels = c("solid" = "n_heads = 12", "dashed" = "n_heads = 8")) +  # Définir les labels de légende pour les types de segments
  theme_minimal() +
